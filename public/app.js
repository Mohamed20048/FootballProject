...
async function loadCompetitions() {
  ...
  document.getElementById('standingsCompetition').innerHTML =
    comps.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
}
...
async function loadMatches() {
    ...
    matches.map(m => `
      <tr>
        ...
        <td>
          ...
          <button data-addgoal="${m.id}">+ Goal</button>
        </td>
      </tr>`).join('');

  table.querySelectorAll('button[data-start]').forEach(b => b.onclick =
    () => api(`/api/matches/${b.dataset.start}/status`, {
      method: 'POST',
  ...
  table.querySelectorAll('button[data-finish]').forEach(b => b.onclick =
    () => api(`/api/matches/${b.dataset.finish}/status`, {
      method: 'POST',
  ...
  table.querySelectorAll('button[data-addgoal]').forEach(b => b.onclick = async () => {
    ...
    const teamIdStr = prompt('Goal for which team ID? (numeric team id)');
    if (!teamIdStr) return;
...
matchForm.onsubmit = async (e) => {
  ...
  try {
    await api('/api/matches', { method: 'POST', body: JSON.stringify(data) });
