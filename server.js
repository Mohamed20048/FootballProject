...
// helper to run .sql files on a DB handle
...
// ---------- CLI flags (init / reset) BEFORE global DB ----------
...
const db = new Database(DB_PATH);
db.pragma('foreign_keys = ON');
// Always ensure schema exists; seed is idempotent thanks to OR IGNORE
runSQL(db, 'schema.sql');
if (fs.existsSync(path.join(__dirname, 'db', 'seed.sql'))) {
  runSQL(db, 'seed.sql');
}
...
const MatchSchema = z.object({
  ...
  date_time: z.string(), // ISO string from datetime-local input
...
app.post('/api/players', requireRole('admin', 'coach'), (req, res) => {
  ...
  try {
    const info = db.prepare(`
      INSERT INTO players (team_id, full_name, position, age, nationality)
      VALUES (?,?,?,?,?)
    `).run(team_id, full_name, position, age, nationality ?? null);
    res.status(201).json({ id: info.lastInsertRowid });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
...
app.put('/api/players/:id', requireRole('admin', 'coach'), (req, res) => {
  const id = Number(req.params.id);
  const allowed = ['team_id', 'full_name', 'position', 'age', 'nationality',
...
app.get('/api/competitions', (req, res) => {
  const rows = db.prepare('SELECT * FROM competitions ORDER BY name').all();
  res.json(rows);
});
...
app.post('/api/competitions', requireRole('admin'), (req, res) => {
  ...
  try {
    const info = db.prepare(`
      INSERT INTO competitions (name, type, min_squad, max_squad, age_limit)
      VALUES (?,?,?,?,?)
...
app.post('/api/competitions/:id/register', requireRole('admin', 'coach'), (req, res) => {
  ...
  const squad = db.prepare(`
    SELECT COUNT(*) as c, MIN(age) as min_age, MAX(age) as max_age
    FROM players WHERE team_id = ?
  `).get(team_id);
  if (squad.c < comp.min_squad)
    return res.status(400).json({ error: `Minimum squad size ${comp.min_squad} not met` });
  if (squad.c > comp.max_squad)
    return res.status(400).json({ error: `Maximum squad size ${comp.max_squad} exceeded` });
  if (comp.age_limit && squad.max_age > comp.age_limit)
    return res.status(400).json({ error: `Age limit ${comp.age_limit} exceeded by some players` });

  try {
    db.prepare('INSERT INTO registrations (team_id, competition_id) VALUES (?,?)')
      .run(team_id, competition_id);
...
app.get('/api/matches', (req, res) => {
  const rows = db.prepare(`
    ...
    ORDER BY datetime(m.date_time) ASC
  `).all();
  res.json(rows);
});
...
app.post('/api/matches', requireRole('admin'), (req, res) => {
  ...
  const {
    ...
    date_time, venue = null, referee = null
  } = parsed.data;
  try {
    const info = db.prepare(`
      INSERT INTO matches (competition_id, home_team_id, away_team_id, date_time, venue, referee)
      VALUES (?,?,?,?,?,?)
    `).run(competition_id, home_team_id, away_team_id, date_time, venue, referee);
    res.status(201).json({ id: info.lastInsertRowid });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
...
app.post('/api/matches/:id/status', requireRole('referee', 'admin'), (req, res) => {
  ...
  if (!['SCHEDULED', 'IN_PLAY', 'FINISHED'].includes(status))
...
app.post('/api/matches/:id/event', requireRole('referee', 'admin'), (req, res) => {
  ...
  if (!['GOAL', 'ASSIST', 'YELLOW', 'RED', 'SUB', 'OWN_GOAL'].includes(type))
    return res.status(400).json({ error: 'Bad event type' });

  const info = db.prepare(`
    INSERT INTO match_events (match_id, minute, type, player_id, team_id, notes)
    VALUES (?,?,?,?,?,?)
  `).run(id, minute, type, player_id, team_id, notes);
  ...
  if (type === 'GOAL' || type === 'OWN_GOAL') {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(homeInc, awayInc, id);
    if (player_id && type === 'GOAL') {
      db.prepare('UPDATE players SET goals = goals + 1 WHERE id = ?').run(player_id);
    }
  }

  if (type === 'YELLOW' && player_id)
    db.prepare('UPDATE players SET yellow_cards = yellow_cards + 1 WHERE id = ?').run(player_id);
  if (type === 'RED' && player_id)
    db.prepare('UPDATE players SET red_cards = red_cards + 1 WHERE id = ?').run(player_id);
...
app.get('/api/fixtures', (req, res) => {
  const rows = db.prepare(`
    ...
    JOIN teams ht ON ht.id = m.home_team_id
    JOIN teams at ON at.id = m.away_team_id
    WHERE datetime(m.date_time) >= datetime('now')
    ORDER BY datetime(m.date_time) ASC
  `).all();
  res.json(rows);
});
...
app.get('/api/standings/:competitionId', (req, res) => {
  ...
  const table = new Map(
    teams.map(t => [
      t.id,
      { team_id: t.id, team: t.name, P: 0, W: 0, D: 0, L: 0, GF: 0, GA: 0, GD: 0, PTS: 0 },
    ])
  );
  ...
  const rows = Array.from(table.values()).sort(
    (a, b) =>
      b.PTS - a.PTS ||
      b.GD - a.GD ||
      b.GF - a.GF ||
